  ...
  """
    ...
    def get_features(self, game_state, action):
        ...
        if len(invaders) > 0 or score > 0 or our_food_left > food_left:
            ...
            #Computes distance to no_invaders
            ...
            if len(no_invaders) == 0:
                ...
                features['no_invader_distance'] = 1000-min(no_inv_dists)
...
class DefensiveReflexAgent(ReflexCaptureAgent):
    ...
    def get_features(self, game_state, action):
        #successor = self.get_successor(game_state, action) # MIRAR ESTO
        #my_state = successor.get_agent_state(self.index)
        #isScared = my_state.scared_timer > 0
        features = util.Counter()
        successor = self.get_successor(game_state, action)
